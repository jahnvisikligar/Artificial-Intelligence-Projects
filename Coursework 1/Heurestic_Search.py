...
def heuristic(gobject, node, goal_zone):
    neighbours = gobject.neighbors(node['label'])
    for n in neighbours:
        out_zone = gobject[node['label']][n]['main_zone']
        if goal_zone == out_zone:
            out_weight = out_weight - 2
        gobject[node['label']][n]['weight'] = out_weight
...
solution = heuristic(data(), 'Euston', 'Victoria')
